...
def getValue_forKey(lang_dict, key, kErrMsg):
    try:
        ...
        print('\tKeyError: {0}'.format(kErrMsg))
        return ''

def getValue(key, fb_dict, act_dict, err_msg_type):
    avalue = ''
    avalue = getValue_forKey(act_dict, key,
                             "{0} did not contain key '{1}' ({2})".format(act_lang,
                                                                          key,
                                                                          err_msg_type))

    if avalue == '':
        avalue = getValue_forKey(fb_dict, key,
                                 "fallback language {0} also did not contain key '{1}' ({2})"
                                 .format(fb_lang, key, err_msg_type))
        if avalue == '':
            print("\tMissingKey: {0} in both active & fallback languages; " \
                  "using key as value ({1})".format(key, err_msg_type))
            avalue = key
        else:
            print('\t Using {0} for translation of key: {1}'.format(fb_lang,
                                                                    key))
...
def parseJStrans(lang, page, lang_dict, js_style, js_objects):
    ...
    page_path = os.path.join(root_dir,
                             'package',
                             'plugin-gargoyle-i18n-{0}'.format(lang),
                             'files',
                             'www',
                             'i18n',
                             lang,
    ...
    if (os.path.exists(page_path)):
        dfileFO = open(page_path, 'rb')
    else:
        page_path_2 = os.path.join(root_dir,
                                   'package',
                                   '*',
                                   'files',
                                   'www',
        ...
        ext_trans_files=glob.glob(page_path_2)
        if ext_trans_files != []:
            dfileFO = open(ext_trans_files[0], 'rb')

    if dfileFO is not None:
        with dfileFO as dfile_fo:
            dtranslines = dfile_fo.readlines()
                ...
                if aline[x] == '<' and aline[x+1] == '%' and aline[x+2] == '~' and aline[x+3] == ' ':
                    ...
                    while y < len(aline):
                        if aline[y] == ' ':
                            key = aline[x + 4: y]
                            if '.' in key:
                                fallback_dict = parseJStrans(fb_lang,
                                                             key.split('.')[0] + '.js',
                                                             fallback_dict,
                                                             False,
                                                             None)
                                active_dict = parseJStrans(act_lang,
                                                           key.split('.')[0] + '.js',
                                                           active_dict,
                                                           False,
                                                           None)
                            ...
                            anewline += getValue(key, fallback_dict,
                                                 active_dict, 'haserl')
                            x = y + 3
                            break
                        y += 1
...
def process_haserl_file(shfile):
    ...
    with open(shfile, 'rb') as shfileFO:
        sf_head = shfileFO.read(21)

        cond1 = sf_head.startswith('\xEF\xBB\xBF#!/usr/bin/haserl')
        cond2 = sf_head.startswith('#!/usr/bin/haserl')
    ...
    if cond1 or cond2:
        with open(shfile, 'wb') as shfileFO:
            shfileFO.seek(0)
...
def find_js_trans_inc_files( jsname ):
    ...
    gdir = os.getcwd()
    gproc = subprocess.Popen("grep -r gargoyle_header_footer -r "+gdir+"/package | grep -v .git | grep .sh",
                             shell=True,
                             cwd=root_dir,
